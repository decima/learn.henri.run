...
> [!NOTE]  Définir les termes suivants : Encoder, Provider, Firewall, Access Control, Role, Voter 

> [!CAUTION] N.B.: On n'utilisera pas FOSUserBundle car ce bundle est inutile.

> [!NOTE]  Qu'est-ce que FOSUserBundle ? Pourquoi ne pas l'utiliser ? 

La sécurité intègre le hash de mot de passe, la connexion et la gestion des autorisations.

> [!NOTE]  Définir les termes suivants : Argon2i, Bcrypt, Plaintext, BasicHTTP 

> [!NOTE]  Expliquer le principe de hachage. 
...
> [!NOTE] Faire un schema expliquant quelle méthode est appelée dans quel ordre dans le `LoginFormAuthenticator`. Définir l'objectif de chaque méthodes du fichier.
